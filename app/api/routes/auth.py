...
def get_current_user(
    ...
    return _get_current_user_from_token(credentials.credentials, db)
...
def flask_register():
    db = SessionLocal()
    try:
        data = _flask_payload(RegisterRequest)
        return jsonify(register(data, db=db))
...
def flask_login():
    db = SessionLocal()
    try:
        data = _flask_payload(LoginRequest)
        return jsonify(login(data, db=db))
    except ValidationError as exc:
        return jsonify({"detail": exc.errors()}), 422
    except HTTPException as exc:
        return _flask_error(exc)
...
def flask_me():
    ...
    try:
        ...
        return jsonify(me(user=user, db=db))
    except HTTPException as exc:
        return _flask_error(exc)
    finally:
        db.close()
...
def flask_delete_me():
    ...
    try:
        user = _get_current_user_from_token(_flask_token(), db)
        return jsonify(delete_user(db=db, user=user))
    except HTTPException as exc:
        return _flask_error(exc)
    finally:
        db.close()
...
@flask_bp.post("/google")
def flask_google_login():
    db = SessionLocal()
    try:
        data = _flask_payload(GoogleLoginRequest)
        return jsonify(login_google(data, db=db))
    except ValidationError as exc:
        return jsonify({"detail": exc.errors()}), 422
    except HTTPException as exc:
        return _flask_error(exc)
...
@flask_bp.post("/forgot-password")
def flask_forgot_password():
    db = SessionLocal()
    try:
        data = _flask_payload(ForgotPasswordRequest)
        return jsonify(forgot_password(data, db=db))
    except ValidationError as exc:
        return jsonify({"detail": exc.errors()}), 422
    except HTTPException as exc:
        return _flask_error(exc)
...
@flask_bp.post("/reset-password")
def flask_reset_password():
    db = SessionLocal()
    try:
        data = _flask_payload(ResetPasswordRequest)
        return jsonify(reset_password(data, db=db))
    except ValidationError as exc:
        return jsonify({"detail": exc.errors()}), 422
    except HTTPException as exc:
        return _flask_error(exc)
...
@flask_bp.post("/forgot-password-otp")
def flask_forgot_password_otp():
    db = SessionLocal()
    try:
        data = _flask_payload(ForgotPasswordRequest)
        return jsonify(forgot_password_otp(data, db=db))
    except ValidationError as exc:
        return jsonify({"detail": exc.errors()}), 422
    except HTTPException as exc:
        return _flask_error(exc)
...
@flask_bp.post("/reset-password-otp")
def flask_reset_password_otp():
    db = SessionLocal()
    try:
