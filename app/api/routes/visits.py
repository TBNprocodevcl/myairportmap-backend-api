...
def flask_get_airports_by_user():
    db = SessionLocal()
    try:
        visited_raw = (request.args.get("visited") or "").strip().lower()
        visited = None
        if visited_raw in {"1", "true", "yes"}:
            visited = True
        elif visited_raw in {"0", "false", "no"}:
            visited = False

        return jsonify(get_airports_by_user(
...
def flask_get_my_visits():
    ...
    try:
        ...
        return jsonify(get_my_visits(
            airport_id=(request.args.get("airport_id") or "").strip() or None,
            city=(request.args.get("city") or "").strip() or None,
            bbox=(request.args.get("bbox") or "").strip() or None,
            skip=_to_int("skip", 0),
...
@flask_bp.get("/me/airports")
def flask_get_my_airports():
    db = SessionLocal()
    try:
        try:
            user = _get_current_user_from_token(_flask_token(), db)
        ...
        return jsonify(get_my_airports(
            visited=visited,
            city=(request.args.get("city") or "").strip() or None,
            bbox=(request.args.get("bbox") or "").strip() or None,
            db=db,
...
@flask_bp.get("/airports/<handle>")
def flask_get_airports_by_handle(handle: str):
    db = SessionLocal()
    try:
        visited_raw = (request.args.get("visited") or "").strip().lower()
        visited = None
        ...
        return jsonify(get_airports_by_handle(
            handle=handle,
            visited=visited,
            city=(request.args.get("city") or "").strip() or None,
            bbox=(request.args.get("bbox") or "").strip() or None,
...
@flask_bp.get("/airports/details")
def flask_get_airport_visit_detail_by_user():
    db = SessionLocal()
    try:
        def _to_int(name: str, default: int):
            raw = (request.args.get(name) or "").strip()
        ...
        return jsonify(get_airport_visit_detail_by_user(
            airport_id=(request.args.get("airport_id") or "").strip(),
            user_id=(request.args.get("user_id") or "").strip(),
            skip=_to_int("skip", 0),
            limit=_to_int("limit", 50),
            sort_by=(request.args.get("sort_by") or "date_visited").strip() or "date_visited",
...
@flask_bp.get("/me/visits/search")
def flask_search_my_visits():
    db = SessionLocal()
    try:
        try:
            user = _get_current_user_from_token(_flask_token(), db)
        ...
        return jsonify(search_my_visits(
            q=(request.args.get("q") or "").strip() or None,
            airport_name=(request.args.get("airport_name") or "").strip() or None,
...
@flask_bp.post("/create/visits")
def flask_create_flight_as_visits():
    db = SessionLocal()
    try:
        try:
            user = _get_current_user_from_token(_flask_token(), db)
        ...
        try:
            payload = FlightCreateRequest.model_validate(raw)
        except ValidationError as exc:
            return jsonify({"detail": exc.errors()}), 422
        return jsonify(create_flight_as_visits(payload=payload, db=db, user=user))
...
@flask_bp.put("/visits/<int:visit_id>")
def flask_update_visit(visit_id: int):
    db = SessionLocal()
    try:
        try:
            user = _get_current_user_from_token(_flask_token(), db)
        ...
        try:
            ...
            return jsonify({"detail": exc.errors()}), 422
        return jsonify(update_visit(visit_id=visit_id, payload=payload, db=db, user=user))
    except HTTPException as exc:
        return jsonify({"detail": exc.detail}), exc.status_code
    finally:
        db.close()
...
@flask_bp.delete("/visits/<int:visit_id>")
def flask_delete_visit(visit_id: int):
    db = SessionLocal()
    try:
        try:
            user = _get_current_user_from_token(_flask_token(), db)
            ...
            return jsonify({"detail": "Invalid token"}), 401
        return jsonify(delete_visit(visit_id=visit_id, db=db, user=user))
    except HTTPException as exc:
        return jsonify({"detail": exc.detail}), exc.status_code
    finally:
        db.close()
...
@flask_bp.post("/import/foreflight")
def flask_import_foreflight_csv():
    db = SessionLocal()
    try:
        try:
            user = _get_current_user_from_token(_flask_token(), db)
        ...
        wrapped = SimpleNamespace(filename=fs.filename, file=fs.stream)
        return jsonify(import_foreflight_csv(file=wrapped, db=db, user=user))
    except HTTPException as exc:
        return jsonify({"detail": exc.detail}), exc.status_code
    finally:
        db.close()
