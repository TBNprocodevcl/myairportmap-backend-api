...
def clerk_fetch_user(user_id: str) -> dict | None:
    ...
    if not CLERK_SECRET_KEY:
        # If you don't set this, we cannot reliably resolve email.
        # Keep explicit failure so you don't think email-locking is working when it isn't.
        raise RuntimeError("CLERK_SECRET_KEY is not set (required to resolve primary email).")
...
def get_email_for_current_user(*, claims: dict | None = None) -> str | None:
    ...
    # 2) Fallback: resolve from Clerk, persist, and lock
    email = clerk_primary_email_for_user_id(user_id)
    if not email:
        return None
...
def verify_clerk_session(req):
    ...
    try:
        # 1) header -> kid
        header = jwt.get_unverified_header(tok) or {}
        kid = (header.get("kid") or "").strip()
        if not kid:
            return None

        # 2) fetch jwks (REAL function)
        jwks = _get_jwks()
        keys = (jwks or {}).get("keys") or []
        jwk_dict = next((k for k in keys if (k.get("kid") or "").strip() == kid), None)
        if not jwk_dict:
        ...
        # 3) construct pem
        key_obj = jwk.construct(jwk_dict, algorithm="RS256")
        pem = key_obj.to_pem().decode("utf-8")

        # 4) decode (we validate issuer manually to normalize trailing slash)
        claims = jwt.decode(
            tok,
            pem,
            algorithms=["RS256"],
        ...
        # 5) manual issuer check with slash-normalization
        issuer_norm = issuer.rstrip("/") if issuer else ""
        token_iss_norm = (claims.get("iss") or "").rstrip("/")
        if issuer_norm and token_iss_norm and token_iss_norm != issuer_norm:
            return None
...
def clerk_sign_in_url(next_path: str = "/logbook") -> str:
    """
    Return the Clerk sign-in URL as a string (NOT a Flask redirect Response).
    ...
    # If we don't have a Clerk issuer, use internal sign-in route
    if not CLERK_ISSUER:
        return f"/sign-in?next={quote(nxt)}"

    # Choose a base URL for redirect_url (APP_BASE_URL preferred)
    base = APP_BASE_URL
    if not base:
        # Best-effort runtime base (works behind proxy)
        scheme = request.headers.get("X-Forwarded-Proto") or request.scheme or "https"
        host = request.headers.get("X-Forwarded-Host") or request.host
        base = f"{scheme}://{host}".rstrip("/")

    return f"{CLERK_ISSUER}/sign-in?redirect_url={quote(base + nxt)}"
...
# ------------------------------------------------------------
# Hosted Clerk sign-up entrypoint (NO local HTML here)
# ------------------------------------------------------------
...
def sign_up_route():
    ...
    # Clean up junk like "/app?" or "/app?&" (prevents next=/app?&fresh=0)
    while nxt.endswith("?") or nxt.endswith("&"):
        nxt = nxt[:-1]
    nxt = nxt.replace("?&", "?")

    fresh = (request.args.get("fresh") or "").strip() == "1"

    clerk_signup = (os.getenv("CLERK_SIGN_UP_URL") or "https://accounts.myairportmap.com/sign-up").strip().rstrip("/")

    # IMPORTANT: return to /auth/finish so we can call /auth/exchange
    finish_url = f"{APP_BASE_URL}/auth/finish?next={quote(nxt)}"
    target = f"{clerk_signup}?redirect_url={quote(finish_url)}"

    # Optional: "fresh sign-up" interstitial to avoid silent loops / to be user-friendly
    if fresh:
        html_out = f"""<!doctype html>
<html lang="en"><head>
  <meta charset="utf-8">
...
<body style="font-family:system-ui;padding:18px;max-width:720px;margin:0 auto;">
  <h2 style="margin:0 0 10px;">Create your account</h2>
  <p style="margin:0 0 16px;opacity:0.85;">Continue to the secure sign-up page.</p>
  <a href="{_html.escape(target)}"
     style="display:inline-block;padding:12px 14px;border-radius:12px;background:#111;color:#fff;text-decoration:none;font-weight:800;">
     Continue to sign up →
  </a>
  <div style="margin-top:14px;font-size:13px;opacity:0.7;">
    <a href="/signed-out" style="color:#111;">Cancel</a>
  </div>
</body></html>"""
        return Response(html_out, mimetype="text/html", status=200)

    # Normal behavior: bounce straight to Clerk
    return redirect(target, code=302)
...
# ------------------------------------------------------------
# Hosted Clerk sign-in entrypoint (spinner bridge on fresh=1)
# ------------------------------------------------------------
...
def sign_in_route():
    ...
    clerk_login = (os.getenv("CLERK_SIGN_IN_URL") or "https://accounts.myairportmap.com/sign-in").strip().rstrip("/")

    base = (APP_BASE_URL or "").strip().rstrip("/") or _runtime_base_url()

    # IMPORTANT: return to /auth/finish so we can call /auth/exchange
    finish_url = f"{base}/auth/finish?next={quote(nxt, safe='/')}"
    target = f"{clerk_login}?redirect_url={quote(finish_url)}"

    if fresh:
        # ✅ Spinner bridge: feels responsive during cross-domain Clerk handoff
        html_out = f"""<!doctype html>
<html lang="en">
<head>
  <meta charset="utf-8">
...
<body>
  <div class="wrap">
    <div class="card">
      ...
      <p class="sub">One moment — opening the secure sign-in page.</p>

      <!-- Fallback if a redirect is blocked -->
      <a class="btn" href="{_html.escape(target)}">Continue to sign in →</a>
      <a class="cancel" href="/signed-out">Cancel</a>
    </div>
  </div>

  <script>
    // Redirect immediately; keep fallback link above for blockers
    window.location.replace("{_html.escape(target)}");
  </script>
  <noscript>
    <meta http-equiv="refresh" content="0;url={_html.escape(target)}">
  </noscript>
</body>
</html>"""
        return Response(html_out, mimetype="text/html", status=200)
...
def _debug_env():
    safe = {
        ...
        "CLERK_SIGN_UP_URL": os.getenv("CLERK_SIGN_UP_URL"),
    }
    return safe, 200
