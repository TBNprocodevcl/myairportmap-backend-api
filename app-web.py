...
from app.api.routes import achievements, airports, auth, certifications, export, runway360, subcription, upload, user
...
# -----------------------------
# ✅ Database-based authentication (replaced Clerk)
# Fetches user data from database instead of Clerk API
...
def get_user_from_db(user_id: str) -> User | None:
    """Fetch user from database by user_id (UUID string)."""
    if not user_id:
        return None
    try:
        db = SessionLocal()
...
def get_email_for_user_id(user_id: str) -> str | None:
    """Get email for a user from database."""
...
def get_email_for_current_user(*, claims: dict | None = None) -> str | None:
    ...
    Gets email directly from database based on user_id (claims["sub"]).
    ...
    return get_email_for_user_id(user_id)


def current_user_email() -> str | None:
    return get_email_for_current_user()

# ✅ JWT-based app configuration (no longer using Clerk)
...
def login_required(fn):
    ...
    def wrapper(*args, **kwargs):
        ...
        if not claims:
            return clerk_sign_in_redirect(request.full_path or "/app")
...
# ============================================================
# Clerk auth helpers (CANONICAL) - uses python-jose (no PyJWT)
# ============================================================
import os
import time
import json
...
# Your first-party cookie (set by /auth/exchange)
APP_SESSION_COOKIE = (os.getenv("APP_SESSION_COOKIE") or "mam_session").strip() or "mam_session"
...
def verify_jwt_session(req):
    """
    Verify JWT token from request using internal SECRET_KEY.
    Returns claims dict if valid, None if invalid.
    
    Replaces Clerk session verification with internal JWT-based auth.
    """
    ...
    try:
        # Use the same decode function as auth.py
        claims = decode_access_token(tok)
        if not claims:
            return None
        
        # Verify user_id (sub) is present
...
def verify_clerk_session(req):
    """
    Backward compatibility wrapper for verify_jwt_session.
    Routes internal JWT verification to replace Clerk.
    """
    return verify_jwt_session(req)
...
def clerk_sign_in_url(next_path: str = "/logbook") -> str:
    """
    Return the internal sign-in URL as a string.
    Uses internal login route instead of Clerk.
    """
    nxt = (next_path or "/logbook").strip() or "/logbook"
    if (not nxt.startswith("/")) or nxt.startswith("//"):
        nxt = "/logbook"

    # Return internal sign-in route
    return f"/sign-in?next={quote(nxt)}"
...
# ------------------------------------------------------------
# SINGLE /auth/exchange route (ONLY ONE copy in the whole file)
# ------------------------------------------------------------
...
# =========================
# Canonical host + Auth entrypoints (Render + Clerk)
# =========================
...
# ------------------------------------------------------------
# Hosted auth sign-up entrypoint (NO local HTML here)
# ------------------------------------------------------------
@app.route("/sign-up", methods=["GET", "POST"])
def sign_up_route():
    """Internal JWT-based registration page (replaced Clerk)."""
    nxt = (request.args.get("next") or request.form.get("next") or "/app").strip() or "/app"
    if not nxt.startswith("/"):
        nxt = "/app"
    ...
    if request.method == "POST":
        ...
        if not email or not password or not first_name:
            ...
            try:
                ...
                auth.register(register_data, db=db)

                login_data = LoginRequest(email=email, password=password)
                login_resp = auth.login(login_data, db=db)

                token = login_resp.get("data", {}).get("access_token") or login_resp.get("access_token")
                if token:
                    resp_obj = redirect(nxt, code=302)
                    resp_obj.set_cookie(APP_SESSION_COOKIE, token, httponly=True, samesite="Lax", secure=True)
                    return resp_obj
...
<head>
  <meta charset="utf-8">
  <meta name="viewport" content="width=device-width, initial-scale=1">
  <title>Create Account - MyAirportMap</title>
  <style>
    * {{ margin: 0; padding: 0; box-sizing: border-box; }}
    body {{
      background: linear-gradient(135deg, #0f1115 0%, #1a1e2e 100%);
      color: #fff;
      font-family: system-ui, -apple-system, Segoe UI, Roboto, sans-serif;
      min-height: 100vh;
      display: flex;
    ...
    .row .form-group {{ flex: 1; margin-bottom: 14px; }}
    .error {{
      background: #3d2424;
      color: #ff6b6b;
    ...
    button {{
      width: 100%;
      padding: 12px 16px;
      background: linear-gradient(135deg, #1f77ff 0%, #1a5fd9 100%);
      color: #fff;
      border: none;
      border-radius: 8px;
      font-weight: 600;
...
<body>
  <div class="container">
    <div class="logo">
      <img src="/static/favicon.png" alt="MyAirportMap">
    </div>
    <h1 class="title">Create Account</h1>
    <p class="subtitle">Join the MyAirportMap community</p>

    {'<div class="error">' + _html.escape(error) + '</div>' if error else ''}
...
def clerk_sign_up_redirect(path: str = "/logbook"):
...
def clerk_sign_up_href(path: str = "/logbook") -> str:
    p = (path or "/logbook").strip() or "/logbook"
...
def _runtime_base_url(req=None) -> str:
    if req is None:
        ...
        try:
            ...
            raise RuntimeError("No request context available for _runtime_base_url()")
    else:
        # FastAPI Starlette request object
        scheme = (req.headers.get("X-Forwarded-Proto") or "").split(",")[0].strip().lower()
        if not scheme:
            scheme = req.url.scheme or "https"
        host = req.headers.get("X-Forwarded-Host") or req.url.hostname or ""

    return f"{scheme}://{host}".rstrip("/")


def _auth_service_url(path: str, req=None) -> str:
    path = (path or "").strip()
    if not path.startswith("/"):
        path = "/" + path
    return _runtime_base_url(req) + path

# ------------------------------------------------------------
# Hosted auth sign-in entrypoint (spinner bridge on fresh=1)
# ------------------------------------------------------------
@app.route("/sign-in", methods=["GET", "POST"])
def sign_in_route():
    print("zo sign-in route")
    """Internal JWT-based login page (replaced Clerk)."""
    nxt = (request.args.get("next") or request.form.get("next") or "/app").strip() or "/app"
    if (not nxt.startswith("/")) or nxt.startswith("//"):
        nxt = "/app"
...
<head>
  <meta charset="utf-8">
  <meta name="viewport" content="width=device-width, initial-scale=1">
  <title>Sign In - MyAirportMap</title>
  <style>
    * {{ margin: 0; padding: 0; box-sizing: border-box; }}
    body {{
      background: linear-gradient(135deg, #0f1115 0%, #1a1e2e 100%);
...
<body>
  <div class="container">
    ...
    <p class="subtitle">Welcome back to MyAirportMap</p>
...
# ------------------------------------------------------------
# Post-Clerk finish step: exchange Clerk token -> app cookie, then forward
# ------------------------------------------------------------
@app.get("/auth/finish")
def auth_finish():
    if request.cookies.get("mam_signed_out") == "1":
        return redirect("/signed-out", code=302)

    nxt = (request.args.get("next") or "/app").strip() or "/app"
    if (not nxt.startswith("/")) or nxt.startswith("//"):
        nxt = "/app"

    pk = (os.getenv("CLERK_PUBLISHABLE_KEY") or "").strip().strip('"').strip("'")
    fapi = (os.getenv("CLERK_FRONTEND_API_URL") or "").strip().strip('"').strip("'").rstrip("/")

    if not pk:
        return Response("CLERK_PUBLISHABLE_KEY is not set.", status=500)
    if not fapi:
        return Response("CLERK_FRONTEND_API_URL is not set.", status=500)

    jwt_template = (os.getenv("CLERK_JWT_TEMPLATE") or "mam_app").strip()

    print("[AUTH_FINISH] hit; next=", nxt, " host=", request.host, " template=", jwt_template)

    base = (APP_BASE_URL or "").strip().rstrip("/") or _runtime_base_url()
    dest = f"{base}{nxt}"
...
<html lang="en"><head>
  ...
  <script crossorigin="anonymous"
    data-clerk-publishable-key="{_html.escape(pk)}"
    src="{_html.escape(fapi)}/npm/@clerk/clerk-js@5/dist/clerk.browser.js?nonce={nonce}"
    type="text/javascript"></script>
</head>
<body>
<div id="s" style="font-family:system-ui;padding:16px;">Signing you in…</div>
<script>
(async function() {{
  ...
  function fail(msg) {{
    s.textContent = msg;
    setTimeout(() => window.location.href = "/signed-out", 500);
  }}
  ...
  try {{
    ...
    try {{
      token = await Clerk.session.getToken({{
        template: "{_html.escape(jwt_template)}",
        skipCache: true
      }});
    }} catch (e) {{
      try {{
    ...
    if (!token) {{
      return fail("Could not fetch Clerk token.");
    }}

    const r = await fetch("/auth/exchange?nonce={nonce}", {{
      method: "POST",
    ...
    return fail("Exchange failed (" + r.status + "): " + t);
    ...
    resp = Response(html_out, mimetype="text/html", status=200)
...
def login_route():
    # Always funnel through the canonical Clerk handoff route
    return redirect("/sign-in?next=/app", code=302)
...
@app.get("/_debug/clerkjs-url")
def debug_clerkjs_url():
    fapi = (os.getenv("CLERK_FRONTEND_API_URL") or "")
    fapi = fapi.strip().strip('"').strip("'").rstrip("/")
    return Response(f"{fapi}/npm/@clerk/clerk-js@5/dist/clerk.browser.js\n", mimetype="text/plain")
  ...
  <div class="wrap">
    ...
    <div class="card">
      ...
      <div class="muted">
        ...
        r2_enabled=<b>{str(dbg_r2)}</b> &middot; key=<span class="mono">{_html.escape(settings_key)}</span><br>
        share_activity=<b>{_html.escape(str(dbg_share_activity))}</b> &middot; legacy public_share_enabled=<b>{_html.escape(str(dbg_public_share_enabled))}</b>
      </div>
      """
      )}
    </div>
...
<script>
  window.addEventListener("load", async () => {{
    try {{
      await Clerk.load();
      await Clerk.signOut({{ redirectUrl: "/signed-out" }});
    }} catch (e) {{
...
""")
    ...
    # Brake cookie blocks /auth/finish loops
    resp.set_cookie(
        "mam_signed_out",
        "1",
        max_age=300,
...
def auth_debug():
    ...
    return jsonify({
        ...
        # ---- Runtime config (JWT-based internal auth) ----
...
async def sign_in_route(request: Request):
    ...
    nxt = (form.get("next") or request.query_params.get("next") or "/app").strip() or "/app"
    if (not nxt.startswith("/")) or nxt.startswith("//"):
        nxt = "/app"
    ...
    if request.method == "POST":
        ...
        if not email or not password:
            ...
            try:
                db = SessionLocal()
                try:
                    auth_data = LoginRequest(email=email, password=password)
                    result = auth.login(auth_data, db=db)
                    token = result.get("data", {}).get("access_token") or result.get("access_token")
                    if token:
                        print("zo day 2")

                        response = RedirectResponse(nxt, status_code=302)
                        response.set_cookie(APP_SESSION_COOKIE, token, httponly=True, samesite="Lax", secure=False)
                        return response
                    error = "Invalid credentials"
                finally:
                    db.close()
...
<head>
  ...
  <title>Sign In - MyAirportMap</title>
  <style>
    body {{ margin:0; min-height:100vh; display:flex; align-items:center; justify-content:center; background:#0f1115; color:#fff; font-family:system-ui, -apple-system, Segoe UI, Roboto, sans-serif; }}
    .card {{ width:min(420px,calc(100vw-32px)); padding:32px; background:#161b23; border:1px solid #2a2f3a; border-radius:18px; box-shadow:0 20px 60px rgba(0,0,0,0.3); }}
    h1 {{ margin:0 0 10px; font-size:26px; }}
    .subtitle {{ margin:0 0 24px; color:#b0b9ca; font-size:14px; }}
    .field {{ margin-bottom:16px; }}
    label {{ display:block; margin-bottom:6px; font-size:13px; color:#d3dae5; }}
    input {{ width:100%; padding:12px 14px; border:1px solid #2d3541; border-radius:12px; background:#10151e; color:#fff; font-size:14px; }}
    .error {{ margin-bottom:16px; padding:12px 14px; border-radius:12px; background:#3e1d1d; color:#ff8b8b; }}
    button {{ width:100%; padding:12px 14px; border:none; border-radius:12px; background:#1f77ff; color:#fff; font-weight:700; cursor:pointer; }}
    .links {{ margin-top:18px; display:flex; justify-content:space-between; gap:12px; font-size:13px; }}
    .links a {{ color:#5faaff; text-decoration:none; }}
  </style>
</head>
<body>
  <div class=\"card\">
    ...
    <div class=\"links\">
      <a href=\"/sign-up?next={_html.escape(nxt)}\">Create account</a>
      <a href=\"/\">Home</a>
    </div>
  </div>
</body>
        ...
        if not email or not password or not first_name:
            ...
            try:
                ...
                try:
                    ...
                    login_data = LoginRequest(email=email, password=password)
                    result = auth.login(login_data, db=db)
                    token = result.get("data", {}).get("access_token") or result.get("access_token")
                    if token:
...
<head>
  <meta charset=\"utf-8\">
  <meta name=\"viewport\" content=\"width=device-width, initial-scale=1\">
  <title>Sign Up - MyAirportMap</title>
  <style>
    body {{ margin:0; min-height:100vh; display:flex; align-items:center; justify-content:center; background:#0f1115; color:#fff; font-family:system-ui, -apple-system, Segoe UI, Roboto, sans-serif; }}
    .card {{ width:min(460px,calc(100vw-32px)); padding:32px; background:#161b23; border:1px solid #2a2f3a; border-radius:18px; box-shadow:0 20px 60px rgba(0,0,0,0.3); }}
    h1 {{ margin:0 0 10px; font-size:26px; }}
    .subtitle {{ margin:0 0 24px; color:#b0b9ca; font-size:14px; }}
    .field {{ margin-bottom:16px; }}
    label {{ display:block; margin-bottom:6px; font-size:13px; color:#d3dae5; }}
...
<body>
  <div class=\"card\">
    ...
    <p class=\"subtitle\">Register a new MyAirportMap account.</p>
    {f'<div class=\"error\">{_html.escape(error)}</div>' if error else ''}
    <form method=\"post\">
      <div class=\"row\">
        <div class=\"field\">
    ...
    <div class=\"links\">
      <a href=\"/sign-in?next={_html.escape(nxt)}\">Already have an account?</a>
      <a href=\"/\">Home</a>
    </div>
  </div>
</body>
