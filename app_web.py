...
def _generate_unique_handle(db, email: str) -> str:
    base_handle = (email or "").split("@", 1)[0].strip().lower() or "pilot"
    handle = base_handle
    suffix = 1
...
def _avatar_url_for_handle(handle: str) -> str:
...
def sign_up_route():
    ...
    if request.method == "POST":
        ...
        if not email or not password or not first_name:
            ...
            try:
                resp = requests.post(
                    _auth_service_url("/auth/register"),
                    json={
                        "email": email,
                        "password": password,
...
def _runtime_base_url() -> str:
    try:
        ...
        try:
            return (request.host_url or "http://localhost").rstrip("/")
        except RuntimeError:
...
def _auth_service_url(path: str) -> str:
    ...
    if not path.startswith("/"):
        path = "/" + path
    return _runtime_base_url() + path

# ------------------------------------------------------------
# Internal JSON API for login (used by /sign-in form)
# ------------------------------------------------------------
@app.route("/auth/login", methods=["POST"])
def auth_login():
    """JSON login endpoint for email/password authentication."""
    from app.db.session import SessionLocal
    from app.models.user import User
...
def auth_register():
    """JSON register endpoint for email/password authentication."""
    from app.db.session import SessionLocal
    from app.models.user import User
    ...
    try:
        ...
        try:
            ...
            handle = _generate_unique_handle(db, email)
            avatar_url = _avatar_url_for_handle(handle)
            user = User(
                email=email,
            ...
            try:
                ...
                if resp.status_code == 200:
                    ...
                    if token:
                        ...
                        response.set_cookie(APP_SESSION_COOKIE, token, httponly=True, samesite="Lax", secure=True)
                        return response
                error = "Invalid credentials"
            except Exception as e:
                error = f"Login error: {str(e)}"
